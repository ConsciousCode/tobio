...
import sys
import time
from typing import Any, ClassVar, ForwardRef, Literal, Optional, Self, Sequence, Union, dataclass_transform, get_args, get_origin, get_type_hints, overload, override
import inspect
...
def to_sql_type(t: Any) -> str:
    ...
    if orig is Literal:
        t = type(args[0]) # Assumes all literals are the same type
    
    if schema := types.get(t):
        return f"{schema} NOT NULL"
    
    raise TypeError(f"Unsupported type {t}")

@dataclass
class Column[T](Descriptor):
    ...
    '''Name of the table the column references.'''
    
    default: Any = NOT_GIVEN
    '''Default value for the column.'''
    
    @overload
    def __get__(self, instance: None, owner: 'Table') -> Self: ...
    @overload
    def __get__(self, instance: 'Row', owner: 'Table') -> T: ...
    
    def __get__(self, instance, owner):
        return self if instance is None else getattr(instance, self.__name__)
    
    def schema(self):
        schema = f"{self.__name__} {to_sql_type(self.type)}"
        ...
        return f"{schema} DEFAULT {dv}"

# Need a helper function which explicitly returns Any so it can be used in
#  type annotations.
def column(annotation: Any, references: Optional[str]=None, default: Any=NOT_GIVEN) -> Any:
    '''Descriptor for a column.'''
    return Column(annotation, references, default)

@dataclass
class Relationship[T](Descriptor):
    ...
    def __get__(self, instance: 'HasId', owner: 'Table'):
        ...
        if self.secondary is None:
            ...
            cur = (instance.db.select(args[0])
                .where(
                    f"rowid IN (SELECT {self.foreign_key} FROM {self.secondary} WHERE {self.local_key} = ?)",
        ...
        if origin is None:
            return hint._bind_new(instance.db, cur.fetchone())
